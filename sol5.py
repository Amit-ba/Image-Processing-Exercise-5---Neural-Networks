...
Original file is located at
    https://colab.research.google.com/drive/1FI2G94iHWGOF-UbFoYgl97GneoNyvwJw
"""
...
#@markdown ###Version 1.0 Last Update 02.01.2021
#@title{ display-mode: "form" }
#@markdown Please enter your cs username:
cs_id = "amitba19" #@param {type:"string"}
...
### just for the record I know that w,h are inversed in this whole code
### but google colab doesn't support refactoring :(

"""# 3 Dataset Handling"""

def get_random_patch(image, crop_size, original=np.zeros((1))):
    x,y = image.shape
    ...
    if(x-crop_x <= 0):
        ...
        random_x = np.random.randint(low=0, high=x-crop_x)

    if(y-crop_y <= 0):
        random_y = 0
    else:
        random_y = np.random.randint(low=0, high=y-crop_y)
    patch = image[random_x:random_x+crop_x, random_y:random_y+crop_y]
    if(np.array_equal(original, np.zeros(1))):
        return patch
    og_patch = original[random_x:random_x+crop_x, random_y:random_y+crop_y]
    return og_patch, patch

def get_corrupted_patches(image, crop_size, corruption_func):
    big_patch = get_random_patch(image, 3*crop_size)
    blurry_big_patch = corruption_func(big_patch)
    small_og_patch, small_blurry_patch = get_random_patch(blurry_big_patch, crop_size, original=big_patch)
    return small_og_patch, small_blurry_patch
...
def load_dataset(filenames, batch_size, corruption_func, crop_size):
    ...
    :return:outputs random tuples of the form (corrupted_batch, original_images_batch), where each output variable is an array of shape(batch_size, height, width, 1).
     original_images_batch is made of clean images and corrupted_batch is their respective randomly corrupted version
     according to corruption_func(im)
    """
    # todo use dictionary + take cropping 3 times of the ctop_size, corrupt, and than crop
    # from corrputed
    cache = {}
    while True:
        original_images_batch, corrupted_batch = [], []
        indexes = np.random.choice(len(filenames), batch_size)
        # filenames_idx = filenames[indexes]        
        # for filename_idx in filenames_idx:
        for index in indexes:
            ...
            small_og_patch, small_corrupted_patch = get_corrupted_patches(image, crop_size, corruption_func)
            original_images_batch.append(small_og_patch - 0.5)
            small_corrupted_patch[small_corrupted_patch < 0] = 0
            corrupted_batch.append(small_corrupted_patch - 0.5)

        original_images_batch = np.expand_dims(original_images_batch, axis=3)
        corrupted_batch = np.expand_dims(corrupted_batch, axis=3)
        x = (np.array(corrupted_batch),np.array(original_images_batch))
        # x = (np.array(original_images_batch),np.array(corrupted_batch))
        yield x
...
def build_nn_model(height, width, num_channels, num_res_blocks):
    ...
    conv  = Conv2D(num_channels, (3,3), padding='same')(input)
    relu = Activation('relu')(conv)
    block = relu
    for i in range(num_res_blocks):
        block = resblock(block, num_channels)
    conv2 = Conv2D(1,(3,3), padding='same')(block)
    add = Add()([input, conv2])
...
def train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples):
    ...
    """
    #divide images to training and validation 80-20 split:
    split_index = int(0.8 * len(images))
    training_im = images[:split_index]
    validation_im = images[split_index:]

    # generate from each set a dataset with the data_generator:
    crop_size = model.input_shape[1:3] #make sure this is giving me the height\width of tensor shape
    training_gen = load_dataset(training_im, batch_size=batch_size, corruption_func=corruption_func, crop_size=crop_size)
    validation_gen = load_dataset(validation_im, batch_size=batch_size, corruption_func=corruption_func, crop_size=crop_size)

    # call compile(), loss = "mean sqaured error", optimizer =Adam(beta_2=0.9)
    adam_opt = Adam(beta_2=0.9)
    model.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer = adam_opt)

    #fit generator to train model:
    model.fit_generator(generator=training_gen, steps_per_epoch=steps_per_epoch,
                        epochs=num_epochs, validation_data=validation_gen,
...
def restore_image(corrupted_image, base_model):
    ...
    """
    # create a model in good size and has same weights as given model:
    w, h = corrupted_image.shape
    a = Input(shape=(w,h,1))
    b = base_model(a)
    new_model = Model(inputs=a, outputs=b)
    new_model.set_weights(base_model.get_weights())

    #use predict + clip + return:
    corrupted_image = corrupted_image - 0.5
    corrupted_image = corrupted_image.reshape((1,w,h,1))
    restored = new_model.predict(x=corrupted_image)[0]
    restored = restored.reshape((w,h))
    restored = restored + 0.5
    restored = np.around(restored * 255)/255 
...
def add_gaussian_noise(image, min_sigma, max_sigma):
    ...
    normalized_random = np.random.normal(0, sigma, image.shape)

    # add noise:
    corrupted = image + normalized_random
...
def learn_denoising_model(denoise_num_res_blocks, quick_mode=False):
    ...
    if(quick_mode): #should work in secongs
        batch_size = 10 
        steps_per_epoch = 3
        num_epoch = 2
    ...
    model = build_nn_model(height=patch_size, width=patch_size, num_channels=channels_num, num_res_blocks=denoise_num_res_blocks)
    train_model(model, images, corruption_func,
                batch_size, steps_per_epoch, num_epoch, num_valid_samples)
...
def random_motion_blur(image, list_of_kernel_sizes):
    ...
    random_index = np.random.randint(len(list_of_kernel_sizes))
    random_kernel = list_of_kernel_sizes[random_index]
    corrupted = add_motion_blur(image, random_kernel, random_angle)
    corrupted = np.around(corrupted * 255)/255
    corrupted = np.clip(corrupted, 0, 1)
    return corrupted
...
def learn_deblurring_model(deblur_num_res_blocks, quick_mode=False):
    ...
    corruption_func = lambda image: random_motion_blur(image, kernel_sizes)
    model = build_nn_model(height=patch_size, width=patch_size, num_channels=channels_num, num_res_blocks=deblur_num_res_blocks)
    train_model(model, images, corruption_func,
                batch_size, steps_per_epoch, num_epoch, num_valid_samples)
    return model
...
def super_resolution_corruption(image):
    ...
    """
    zoom = np.random.randint(low=2, high=5)
    corrupted = ndimage.zoom(image,1/zoom)
    corrupted = ndimage.zoom(corrupted,(np.array(image.shape))/(np.array(corrupted.shape)))
    corrupted = np.around(corrupted * 255)/255
    corrupted = np.clip(corrupted, 0, 1)
    return corrupted
...
def learn_super_resolution_model(super_resolution_num_res_blocks, quick_mode=False):
    ...
    corruption_func = lambda image: super_resolution_corruption(image)
    model = build_nn_model(height=patch_size, width=patch_size, num_channels=channels_num, num_res_blocks=deblur_num_res_blocks)
    train_model(model, images, corruption_func,
                batch_size, steps_per_epoch, num_epoch, num_valid_samples)
    return model
...
Answer1 = "I made a corrpution function by zooming in and than zooming out, leaving us with  a lower resolution image.  Afterwards I have made sure the image size stayed the same using intrepolation." #@param {type:"string"}
...
Answer2 = "I have made a scripts that randomlly tried many different parameters and then chose the one with the best result. As much as we learned in class - much of the progress with NN was made by trial and error..." #@param {type:"string"}
...
"""##7.4 Effect of depth
**Note:** You should save the plots to the paths "./depth_plot_denoise.png" and "./depth_plot_deblur.png"
"""

# from google.colab import files
def depth_helper(func, figname):
    print(figname)
    plt.figure()
    for i in range(1,6): #1 to 5 inclusive        
        label = str(i) + " ResNet Blocks"
        print("Now training " + label + " of " + figname)
        model = func(i)
        plt.plot(model.history.history['val_loss'], label=label)
    plt.title(figname)
    plt.legend()
    plt.savefig(figname)
    plt.show()
...
def effect_of_depth():
    ...
    """
    deblur = lambda i: learn_deblurring_model(i, False)
    denoise = lambda i: learn_deblurring_model(i, False)
    sr = lambda i: learn_super_resolution_model(i, False)
    funcs = [denoise, deblur, sr]
    names = ["./depth_plot_denoise.png","./depth_plot_deblur.png","./depth_plot_super_resolution.png"]

    for i in range(3):
        print(" ************* [NOW TRAINING " + names[i] +" ] ************* ")
        depth_helper(funcs[i], names[i])
...
Answer3 = "As we can see - more blocks equals more parameters thus the fit is getting better and better, until a point where it's useless. I have read that 1000 blocks resnet is barley better than 100 blocks resnet (tried neither from obvious reasons)" #@param {type:"string"}
...
def one_image_generator(noise, corrupted, batch_size):
    w,h, z = noise.shape
    while True:
        # noise = noise.reshape(w,h,1)
        # corrupted = corrupted.reshape(w,h,1)
        noise_batch = np.full(shape=(batch_size,w,h,1),fill_value=noise)
        corrupted_batch = np.full(shape=(batch_size,w,h,1), fill_value=corrupted)
        yield (noise_batch, corrupted_batch)

def resblock_deep_prior(input_tensor, num_channels):
    """
    Takes as input a symbolic input tensor and the number of channels for each of its convolutional layers, and returns the symbolic output tensor of the resnet block.
    The convolutional layers should use “same” border mode, so as to not decrease the spatial dimension of the output tensor.
    :param input_tensor: input tensor
    ...
    # maybe no need for input??
    conv1 = Conv2D(num_channels,(3,3), padding='same')(input_tensor)
    relu1 = layers.LeakyReLU(alpha=0.1)(conv1)
    conv2 = Conv2D(num_channels,(3,3), padding='same')(relu1)
    addition = Add()([input_tensor, conv2])
    relu2 = layers.LeakyReLU(alpha=0.1)(addition)
    return relu2

def build_nn_model_deep_prior(height, width, num_channels, num_res_blocks):
    """
    Create an untrained Keras model with input dimension the shape of (height, width, 1), and all convolutional layers (including residual
    blocks) with number of output channels equal to num_channels, except the very last convolutional layer which should have a single output channel.
    The number of residual blocks should be equal to num_res_blocks.
    ...
    input = Input(shape=(height, width, 1))
    conv  = Conv2D(num_channels, (3,3), padding='same')(input)
    relu = layers.LeakyReLU(alpha=0.1)(conv)
    block = relu
    for i in range(num_res_blocks):
        block = resblock_deep_prior(block, num_channels)
    conv2 = Conv2D(1,(3,3), padding='same')(block)
    add = Add()([input, conv2])
    return Model(inputs=input, outputs=add)
...
def deep_prior_restore_image(corrupted_image):
    ...
    """
    noise = np.random.rand(64,64,1) # we give the nn this exact noise in each iteration
    noise = noise * (1/10) # as they said in article
    corrupted_image = corrupted_image.reshape(64,64,1)
    patch_size = 64 
    channels_num = 32
    batch_size = 100
    steps_per_epoch = 100
    ...
    model = build_nn_model_deep_prior(height=patch_size, width=patch_size, 
                                      num_channels=channels_num, num_res_blocks=num_res_blocks)
    adam_opt = Adam()
    for i in range(num_epoch):
        generator = one_image_generator(noise, corrupted_image, batch_size)
        model.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer = adam_opt)
        model.fit_generator(generator=generator, steps_per_epoch=steps_per_epoch,
                            epochs=1)

        resshaped_noise = noise.reshape(1,64,64,1)
        restored = model.predict(x=resshaped_noise)[0]
        restored = restored.reshape(64,64)
        show_corr = corrupted_image.reshape(64,64)
        # IMPORTANT!!!
        # uncomment to view how number of iterations impacts the result!
        show_two_images(restored, show_corr, restored - show_corr)
    
    noise = noise.reshape(1,64,64,1)
    plt.imshow(noise.reshape(64,64))
    plt.show()
    restored = model.predict(x=noise)[0]
    restored = restored.reshape(64,64)
    restored = restored.astype('float64') 
    return restored


# ======================== SOME SMALL TESTING FUNCTIONS IF YOU WANT TO USE THEM :) =======

# test_bonus()


def show_four_images(one, two, three, four):
...
def get_image():
    ...
    filename = filenames[121]
    image = read_image(filename, 1)
    im = image [100:164, 100:164]
    return im


def test_generator():
    im = get_image()
    corrupted = add_gaussian_noise(im, 0, 0.2)
    corrupted = corrupted.reshape(64,64,1)
    batch_size = 10
    noise = np.random.rand(64,64,1) 
    y = one_image_generator(noise, corrupted, batch_size)
    x = next(y)
    for i in range(len(x)):
        ...
        corrupted = corrupted.reshape(64,64)
        show_two_images(noise, corrupted, corrupted)


def show_corr():
    im = get_image()
    x = add_gaussian_noise(im, 0, 0.1)
    show_two_images(im, x,x)


def test_bonus():
    image = get_specific_patch(103,95,217)
    corrupted = add_gaussian_noise(image, 0, 0.1)
    restored = deep_prior_restore_image(corrupted.copy())
    show_four_images(image, restored, corrupted, image-restored)


def show_two_images(one, two, three):
...
def get_random_image():
    filenames = images_for_denoising()
    random_index = np.random.randint(low=0, high=len(filenames))
    ...
    if(y-crop_y <= 0):
        ...
        random_y = np.random.randint(low=0, high=y-crop_y)
    patch = image[random_x:random_x+crop_x, random_y:random_y+crop_y]
    print(random_index, random_x, random_y)
    return patch

def get_specific_patch(random_index, random_x, random_y):
    filenames = images_for_denoising()
    filename = filenames[random_index]
    image = read_image(filename, 1)
    crop_x, crop_y = 64, 64
...
# x = get_specific_patch(103, 95, 217)
# plt.imshow(x)
# plt.show()
